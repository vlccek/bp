...
class boxSplitTest : public testing::TestWithParam<std::pair<float, float>> {
...
TEST_P(boxSplitTest, postiveNumber) {
  ...
  std::cout << std::format("Testing splitBoxBy8 with min: {} and max: {}",
                           GetParam().first, GetParam().second);
  float min = GetParam().first;
  float max = GetParam().second;
  ...
  for (auto i : a) {
    for (const auto &it : splitedbox) {
      if (i == it) {
        splitedbox.erase(std::remove(splitedbox.begin(), splitedbox.end(), it),
                         splitedbox.end());
        continue;
      }
    }
  ...
  for (auto i : splitedbox) {
    std::cout << std::format("{} {} {} x {} {} {} ", i.min.x, i.min.y, i.min.z,
...
TEST_P(boxSplitTest, negativeNumber) {
  float min = GetParam().first * (-1);
  float max = GetParam().second * (-1);
  std::cout << std::format("Testing splitBoxBy8 with min: {} and max: {}", min,
                           max);
...
INSTANTIATE_TEST_SUITE_P(PositiveNumber, boxSplitTest,
                         testing::Values(std::pair<float, float>{0, 1},
                                         std::pair<float, float>{0, 2},
                                         std::pair<float, float>{10, 20},
                                         std::pair<float, float>{100, 200},
                                         std::pair<float, float>{111.50, 200.5},
                                         std::pair<float, float>{0.00000,
                                                                 0.000000006}
...
TEST(Box, isInsideVoronoi) {
  Box b = Box({0, 0, 0}, {1, 1, 1});
  std::vector<Point> vorocell = {Point(1, 1, 1), Point(0, 50, 4),
                                 Point(0, 0, 0), Point(100, -5, 10)};
